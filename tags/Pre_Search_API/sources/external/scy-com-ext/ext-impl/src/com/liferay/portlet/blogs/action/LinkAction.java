	...
	public void processAction(ActionMapping mapping, ActionForm form, PortletConfig portletConfig, ActionRequest req, ActionResponse res) throws Exception {
		...
		try {
			...
			if (cmd == null) {
				...
				// set Forward
				setForward(req, "portlet.blogs.view_link");
				return;
				...
				String classNameIdBlog = String.valueOf(ClassNameLocalServiceUtil.getClassNameId(BlogsEntry.class.getName()));
				// add link for blog
				util.addLink(entryId.toString(), linkedResourceId.toString(), classNameIdBookmark, serviceContext);
				// add link for new bookmark
				util.addLink(linkedResourceId.toString(), entryId.toString(), classNameIdBlog, serviceContext);
