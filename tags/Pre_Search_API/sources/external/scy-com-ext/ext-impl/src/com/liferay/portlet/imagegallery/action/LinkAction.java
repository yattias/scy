...
/**
 * Extern-Link action for similarity image portlet. Add extern links from a
...
public class LinkAction extends PortletAction {
	...
	public void processAction(ActionMapping mapping, ActionForm form, PortletConfig portletConfig, ActionRequest req, ActionResponse res) throws Exception {
		...
		try {
			entry = IGImageLocalServiceUtil.getIGImage(entryId);
			ServiceContext serviceContext = ServiceContextFactory.getInstance(IGImage.class.getName(), req);
			if (cmd == null) {
				...
				// set Forward
				setForward(req, "portlet.image_gallery.view_link");
				return;
				...
				String classNameIdImage = String.valueOf(ClassNameLocalServiceUtil.getClassNameId(IGImage.class.getName()));
				util.addLink(entryId.toString(), linkedResourceId.toString(), classNameIdBookmark, serviceContext);
				util.addLink(linkedResourceId.toString(), entryId.toString(), classNameIdImage, serviceContext);
