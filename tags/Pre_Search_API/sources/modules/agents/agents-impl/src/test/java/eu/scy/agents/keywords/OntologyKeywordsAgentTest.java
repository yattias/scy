...
public class OntologyKeywordsAgentTest extends AbstractTestFixture {
	...
	public void setUp() throws Exception {
		...
		this.agentMap.put(OntologyKeywordsAgent.class.getName(), params);
	...
	public void testRun() throws TupleSpaceException {
		...
		this.getCommandSpace().write(
				new Tuple(OntologyKeywordsAgent.EXTRACT_ONTOLOGY_KEYWORDS, AgentProtocol.QUERY, queryId.toString(),
						TEXT));
		Tuple response = this.getCommandSpace().waitToRead(
				new Tuple(OntologyKeywordsAgent.EXTRACT_ONTOLOGY_KEYWORDS, AgentProtocol.RESPONSE, queryId.toString(),
						String.class), AgentProtocol.ALIVE_INTERVAL * 4);
		assertNotNull("no response received", response);
		String keywords = (String) response.getField(3).getValue();
		assertEquals("wrong keywords", "", keywords);
	}
}
