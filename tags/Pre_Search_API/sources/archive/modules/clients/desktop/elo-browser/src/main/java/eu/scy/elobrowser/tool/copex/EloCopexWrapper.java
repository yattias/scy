 ...
 */
public class EloCopexWrapper {
	private static final Logger logger = Logger.getLogger(EloCopexWrapper.class.getName());
	public static final String untitledDocName = "untitled";
    public static final String scyCopexType = "scy/xproc";
   ...
   public String getDocName()
   {
	...
	public void loadDataProcessAction()
	{
		...
		IMetadataQuery<IMetadataKey> metadataQuery = new BasicMetadataQuery<IMetadataKey>(typeKey,
					BasicSearchOperations.EQUALS, scyCopexType, null);
		query = metadataQuery;
		List<ISearchResult> searchResults = repository.search(query);
		URI[] xprocUris = new URI[searchResults.size()];
		int i = 0;
		for (ISearchResult searchResult : searchResults)
			xprocUris[i++] = searchResult.getUri();
       
		URI xprocUri = (URI) JOptionPane.showInputDialog(null, "Select xproc", "Select xproc",
					JOptionPane.QUESTION_MESSAGE, null, xprocUris, null);
		if (xprocUri != null)
		{
			loadElo(xprocUri);
		}
	...
	public void loadElo(URI eloUri)
	{
		...
		if (newElo != null)
		{
			String eloType = newElo.getMetadata().getMetadataValueContainer(typeKey).getValue()
						.toString();
			if (!scyCopexType.equals(eloType) )
	...
	public void saveDataProcessAction()
	{
		...
		{
			elo.getContent().setXmlString(jdomStringConversion.xmlToString(copexPanel.getExperimentalProcedure()));
	...
	{
		logger.fine("save as experimental procedure");
		String drawingName = JOptionPane.showInputDialog("Enter experimental procedure name:", docName);
        if (StringUtils.hasText(drawingName))
		{
			...
			elo.getMetadata().getMetadataValueContainer(typeKey).setValue(scyCopexType);
