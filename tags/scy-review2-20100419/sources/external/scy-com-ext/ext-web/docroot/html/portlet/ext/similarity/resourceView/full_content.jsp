...
<%
List results = (List)request.getAttribute("viewSim.jsp-results");

int assetIndex = ((Integer)request.getAttribute("viewSim.jsp-assetEntryIndex")).intValue();

TagsAsset asset = (TagsAsset)request.getAttribute("viewSim.jsp-assetEntry");

String title = (String)request.getAttribute("viewSim.jsp-title");
...
boolean show = ((Boolean)request.getAttribute("viewSim.jsp-show")).booleanValue();

request.setAttribute("viewSim.jsp-showIconLabel", true);

String maxWindowState = WindowState.MAXIMIZED.toString();
String themeWindowState = renderRequest.getWindowState().toString();
PortletURL viewFullContentURL;
String viewFullContent = StringPool.BLANK;
...
<div class="asset-full-content <%= showAssetTitle ? "show-asset-title" : "" %>">
	<c:choose>
		<c:when test="<%= className.equals(BlogsEntry.class.getName()) %>">
			...
			viewFullContentURL = renderResponse.createRenderURL();
			...
			viewFullContent = viewFullContentURL.toString();
			...
			<c:choose>
				<c:when test="<%= showAssetTitle %>">
					<h3 class="asset-title <%= AssetPublisherUtil.TYPE_BLOG %>">
						<liferay-util:include page="/html/portlet/ext/similarity/resourceView/edit_actionStartResource.jsp" />
						<%= title %>
					</h3>
				</c:when>
				<c:otherwise>
		...
		<c:when test="<%= className.equals(FreestylerEntry.class.getName()) %>">
			<c:choose>
				<c:when test="<%= showAssetTitle %>">
					<h3 class="asset-title freestyler">
						<liferay-util:include page="/html/portlet/ext/similarity/resourceView/edit_actionStartResource.jsp" />
			...
			<div class="asset-content">
				<div class="content-body">
					...
					FreestylerEntry entry = freestylerEntry;
				
					TagsAssetLocalServiceUtil.incrementViewCounter(className, freestylerEntry.getFreestylerId());
		
					viewFullContentURL = new PortletURLImpl(request, "freestyler", plid, PortletRequest.ACTION_PHASE);
					viewFullContentURL.setWindowState(WindowState.MAXIMIZED);
					viewFullContentURL.setPortletMode(PortletMode.VIEW);
					viewFullContentURL.setParameter("struts_action", "/ext/freestyler/view_entry");
					viewFullContentURL.setParameter("freestylerEntryId", String.valueOf(freestylerEntry.getFreestylerId()));

					viewFullContent = viewFullContentURL.toString();		
					
					List<FreestylerImage> feImageList = FreestylerEntryLocalServiceUtil.getFreestylerImages(entry.getFreestylerId());
			
					request.setAttribute("freestyler_entry", entry);
					if(feImageList.size() > 0){
						request.setAttribute("actualImage", feImageList.get(0));					
					}
					
					if(themeWindowState.equals(maxWindowState)){
						request.setAttribute("windowStateMax", true);							
					}else{
						request.setAttribute("windowStateMax", false);							
					...
					<div class="asset-content">
							<liferay-util:include page="/html/portlet/ext/freestyler/freestylerImageView.jsp" />
					</div>

					<c:if test="<%= showContextLink %>">
						<div class="asset-more">
							<a href="<%= viewFullContent %>" target="_blank"><liferay-ui:message key="view-freestyler" /> &raquo;</a>
						</div>
					</c:if>
...
<%
			...
			viewFullContentURL.setPortletMode(PortletMode.VIEW);
			viewFullContentURL.setParameter("struts_action", "/ext/freestyler/view_entry");
			viewFullContentURL.setParameter("freestylerEntryId", String.valueOf(feEntry.getFreestylerId()));

			viewFullContent = viewFullContentURL.toString();		
%>			
								<h3 class="asset-title freestyler">
									Bild: <%= feImage.getName()%> von <%= feEntry.getName()%>
								</h3>
			<liferay-util:include page="/html/portlet/ext/freestyler/freestylerImageView.jsp" />
			<div class="asset-more">
				<a href="<%= viewFullContent %>" target="_blank"><liferay-ui:message key="view-freestyler" /> &raquo;</a>
