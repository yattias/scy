...
import de.fhg.iais.kd.tm.obwious.operator.system.feature.atomic.ProvideTokens;
import de.fhg.iais.kd.tm.obwious.type.Container;
import eu.scy.agents.impl.elo.AbstractELOAgent;

public class ObwiousAgent<T extends IELO<K>, K extends IMetadataKey> extends
	...
	class TestObwious extends Workflow {
		...
		public TestObwious() {
			...
			this.addOperatorSpecification("ELO2OBWIOUSDocument",
					ELO2OBWIOUSDocument.class);
			this.addNamespaceLink("ELO2OBWIOUSDocument", "elo", "elo");
			this.setInputParameter("ELO2OBWIOUSDocument", "locale",
					Locale.GERMAN);
			...
			this.addOperatorSpecification("TermFrequencies",
	...
	public void processElo(T elo) {
		...
		if (tokenKey == null) {
			K token = (K) new StringMetadataKey("token",
		...
		if (workflowKey == null) {
			K workflowK = (K) new StringMetadataKey("workflow",
		...
		IMetadataValueContainer tokenContainer = elo.getMetadata()
				.getMetadataValueContainer(tokenKey);
		tokenContainer.setValueList((List<String>) doc
				.getFeature(Features.TOKENS));

		IMetadataValueContainer workflowContainer = elo.getMetadata()
				.getMetadataValueContainer(workflowKey);
