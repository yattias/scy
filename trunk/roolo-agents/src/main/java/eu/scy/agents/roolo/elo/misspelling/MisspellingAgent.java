...
public class MisspellingAgent extends AbstractProcessingAgent {
    ...
    protected void doRun() {
        Tuple t;
        try {
            t = getTupleSpace().waitToTake(new Tuple("misspellings", Long.class, String.class));
        } catch (TupleSpaceException e) {
            throw new RuntimeException(e);
        }
        
        String content = (String) t.getField(2).getValue();
        ...
        try {
            ...
            if (numOfErrors > 0) {
                getTupleSpace().write(new Tuple("misspellings", System.currentTimeMillis(), numOfErrors));
            }
        } catch (Exception e) {
            e.printStackTrace();
            throw new RuntimeException(e);
