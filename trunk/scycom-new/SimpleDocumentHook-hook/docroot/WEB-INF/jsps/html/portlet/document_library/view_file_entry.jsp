...
function createPicFull(imgUrl) {
  var newPage = "<html><head><title>"
  newPage += "Preview";
  newPage += "</title></head><body>";
  newPage += "<img src=\"" + imgUrl + "\" />";
  newPage += "</body></html>";
  var j = window.open('')
...
String[] pictureExtensions = new String[] {"JPG", "JPEG", "PNG", "GIF"};

boolean isPicture = false;
for (String picExtension : pictureExtensions) {
	if (extension.toUpperCase().equals(picExtension)) {
		isPicture = true;
		break;
	}
}
...
<aui:layout>
	<aui:column columnWidth="100" cssClass="lfr-asset-column lfr-asset-column-details" first="<%= true %>">
		<liferay-ui:icon image="../arrows/01_down" message="add-document" url="<%= fileUrl %>" />
		...
		<c:if test="<%= showActions && DLFileEntryPermission.contains(permissionChecker, fileEntry, ActionKeys.UPDATE) %>">
			...
			<liferay-ui:icon
				...
				url="<%= editURL %>"
			/>
			<a href="<%= editURL %>"><liferay-ui:message key="edit" /></a>&nbsp;
		</c:if>
		...
		<c:if test="<%= isPicture %>">
			<div class="dl-pic-preview">
				<a onclick="createPicFull('<%= fileUrl %>')">
					<img src="<%= fileUrl %>" />
				</a>
			</div>
		</c:if>
