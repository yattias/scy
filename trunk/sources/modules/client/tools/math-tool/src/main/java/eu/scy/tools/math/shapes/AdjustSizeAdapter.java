...
public class AdjustSizeAdapter extends MouseAdapter {
	...
	public void mousePressed(MouseEvent event) {
		...
		for (IMathShape shape : mathShapes) {
			...
			if( i != -1) {
				System.out.println("adjust contains found it!! " + foundShape);
				foundShape = shape;
				position = i;
				return;
			}
...
//		}
    ...
    public void mouseMoved(MouseEvent event) {
    	...
    	Point p = event.getPoint();
    	 ...
    	 if(!isOverRect(p,foundShape)) {
	            if(((JComponent) this.shapeCanvas).getCursor() != Cursor.getDefaultCursor()) {
	                // If cursor is not over rect reset it to the default.
	                ((JComponent) this.shapeCanvas).setCursor(Cursor.getDefaultCursor());
	            }
	            return;
    	}
	            ...
	            case Rectangle.OUT_BOTTOM + Rectangle.OUT_RIGHT:
	                if(Math.abs(p.x - (r.x+r.width)) < PROX_DIST &&
	                   Math.abs(p.y - (r.y+r.height)) < PROX_DIST) {
	                    ((JComponent) this.shapeCanvas).setCursor(Cursor.getPredefinedCursor(
    ...
    private int getOutcode(Point p, IMathShape mathShape) {
        Rectangle r = (Rectangle)mathShape.getCornerPointRectangle().clone();
