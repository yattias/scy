...
public class Logger implements ActionListener, IDataClient {
	...
	public Logger(DataServer dataServer) {
		...
		inputVariableTimer = new Timer(500, new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				action.addAttribute("name", lastInputVariableValueChangedAction
						.getAttribute("name"));
				action.addAttribute("oldValue",
						firstInputVariableValueChangedAction
								.getAttribute("oldValue"));
				action.addAttribute("newValue",
						lastInputVariableValueChangedAction
		...
		outputVariableTimer = new Timer(500, new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				action.addAttribute("name",
						lastOutputVariableValueChangedAction
								.getAttribute("name"));
				action.addAttribute("oldValue",
						firstOutputVariableValueChangedAction
								.getAttribute("oldValue"));
				action.addAttribute("newValue",
						lastOutputVariableValueChangedAction
								.getAttribute("newValue"));
	...
	private ArrayList<ModelVariable> getVariables(int mv) {
		...
		for (ModelVariable variable : dataServer.getVariables("")) {
			if (variable.getKind() == mv) {
				System.out.println("Logger added variable: "
						+ variable.getName() + " type: " + variable.getKind());
	...
	public void updateClient() {
		for (int i = 0; i < inputVariables.size(); i++) {
			if (inputVariables.get(i).getValue() != oldInputValues.get(i)) {
				logValueChanged(inputVariables.get(i).getName(), oldInputValues
						.get(i), inputVariables.get(i).getValue());
			}
		}
		for (int i = 0; i < outputVariables.size(); i++) {
			if (outputVariables.get(i).getValue() != oldOutputValues.get(i)) {
				logValueChanged(outputVariables.get(i).getName(),
						oldOutputValues.get(i), outputVariables.get(i)
								.getValue());
			}
		}
		storeOldValues();
	...
	public void logAddRow(DataSetRow newRow) {
		...
		action.addAttribute("dataRow", xmlOutputter
				.outputString(newRow.toXML()));
		write(action);
	}
