...
public class DrawPanel extends javax.swing.JPanel {
    ...
    // parami申tres de la zone de traci申
    ...
    // parami申tres du rectangle de zoom
    private int x_zoom1 ;
    private int y_zoom1 ;
    private int x_zoom2 ;
    private int y_zoom2;
    ...
    /* indique si dessine le carr辿 de zoom */
    ...
    /** mi申thode pour retracer inti申gralement la zone de traci申
     * penser i申 appeler la methode effacer() avant cette methode si besoin
     */
    public void tracerZone(Graphics g) {
        ...
        // traci申 des axes et des graduations
        tracerAxes(g);

        // parcours et trac辿 pour toutes les courbes d辿finies dans la HashMap
        ...
        // parcours et traci申 de tous les points di申finis dans le tableau de donni申es
        ...
        // affichage coord
        tracerCoord(g);
        // MaJ des parami申tres de distance
    ...
    /** mi申thode permettant de ri申aliser le traci申 d'un point (croix ou cercle) avec une certaine couleur*/
    public void tracerPoint(Graphics g, Color couleur, String type, double x, double y) {

        // les coordoni申es des segments i申 tracer (coordonni申es de l'i申cran)
        int xt = xToXEcran(x) ;
        int yt = yToYEcran(y) ;
    ...
    /** mi申thode permettant de ri申aliser le traci申 d'un segment de droite avec une certaine couleur*/
    public void tracerSegment(Graphics g, Color couleur, double x1, double y1, double x2, double y2) {
     ...
     /** Traci申 des graduations et des axes du graphique */
    public void tracerAxes(Graphics g) {

        int k0 ;
        int k1 ;

        // traci申 des graduations
        // axe des X
        ...
        // traci申 des axes
        // axe des X
        if (y_min*y_max<=0) { // la zone de traci申 contient l'axe des x
        ...
        // axe des Y
        if (x_min*x_max<=0) { // la zone de traci申 contient l'axe des y
     ...
     /** m辿thode permettant de r辿aliser le trac辿 d'une courbe avec une certaine couleur*/
    public void tracerUneCourbe(Graphics g, Color couleur, Expression fonction) {
        // les coordon辿es des segments (coordonn辿es du rep竪re)
        double x1;
        double x2;
        double y1;
        double y2;
        ...
        while (x1 < x_max) {
            // on calcule 2 fois plus de points que la largeur en pixels de la zone de trac辿
            x2 = x1 + (x_max-x_min)/(width*2) ;
            y2 = fonction.valeur(x2) ;

            if ( !( (Double.isNaN(y1)) || (Double.isNaN(y2)) // 辿vite les valeurs qui ne sont pas d辿finies
            || (y1<y_min && y2>y_max)  || (y1>y_max && y2<y_min) // 辿vite les raccord d'infinis
            || (Double.isInfinite(y1)) || (Double.isInfinite(y2)) // evite de faire des tarc辿s pour des valeurs infinies
            ))
...
/*************************   Mi申thodes de traitements mathematiques **************************/
    ...
    /** retourne un double avec le nombre de chiffres significatifs souhaiti申s */
    ...
    private void zoneDeTraceMouseDragged(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_zoneDeTraceMouseDragged
        // met i申 jour l'affichage des coordonni申es
        zoneDeTraceMouseMoved(evt);
        if(getMousePosition() == null)
            return;
        if(graphMode == DataConstants.MODE_ZOOM){
            // efface le pri申ci申dent rectangle
            isZoom = false;
            repaint();
            //g.setColor(Color.WHITE) ;
            //g.drawRect(Math.min(x_zoom1,x_zoom2) , Math.min(y_zoom1,y_zoom2) , Math.abs(x_zoom1-x_zoom2) , Math.abs(y_zoom1-y_zoom2)) ;
            ...
            // affiche le rectangle correspondant i申 la zone de zoom
            isZoom = true;
            repaint();
...
//            fitexPanel.setCoordY(format.format(y));
    ...
    private void zoneDeTraceMousePressed(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_zoneDeTraceMousePressed
        if (getMousePosition() == null)
            return;
        if(graphMode == DataConstants.MODE_ZOOM){
            // affiche en rouge les coordoni申es
    ...
    private void zoneDeTraceMouseReleased(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_zoneDeTraceMouseReleased
        // ri申affiche en noir les coordoni申es
