...
public class SyncMessageTestCase {
    ...
    private SyncMessage getTestSyncMessage() {
        SyncMessage sm = new SyncMessage();
        sm.setContent(TEST_CONTENT);
        sm.setEvent(TEST_EVENT);
        sm.setToolId(TEST_TOOL_ID);
        sm.setToolSessionId(TEST_TOOL_SESSION_ID);
        sm.setFrom(TEST_FROM);
        return sm;
    }
    ...
    public void testConvertToXMPPMessage() {
        ...
        SyncMessage syncMessage = getTestSyncMessage();
