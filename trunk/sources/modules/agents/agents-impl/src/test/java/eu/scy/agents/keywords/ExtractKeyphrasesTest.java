...
public class ExtractKeyphrasesTest extends AbstractTestFixture {
  ...
  public void testRun() throws TupleSpaceException {
    ...
    Tuple query = new Tuple(ExtractKeyphrasesAgent.EXTRACT_KEYPHRASES, AgentProtocol.QUERY,
                            queryId.toString(), TEXT, MISSION1, LANGUAGE);
    getCommandSpace().write(query);
    System.out.println(query.toString());
    Tuple response = getCommandSpace().waitToRead(new Tuple(
                                                            ExtractKeyphrasesAgent.EXTRACT_KEYPHRASES,
                                                            AgentProtocol.RESPONSE,
                                                            queryId.toString(), String.class),
