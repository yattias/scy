...
public class GroupFormationAgent extends AbstractRequestAgent implements
    ...
    private Set<String> getAvailableUsers(GroupFormationScope scope,
        ...
        switch (scope) {
            case LAS:
                availableUsers = getUsers(UserLocationAgent.METHOD_USERS_IN_LAS, mission, las);
                break;
            case MISSION:
                availableUsers = getUsers(UserLocationAgent.METHOD_USERS_IN_MISSION, mission, las);
                break;
        }
    ...
    private Set<String> getUsers(String method, String mission, String las) {
        Tuple request = new Tuple(UserLocationAgent.USER_INFO_REQUEST, AgentProtocol.QUERY, new VMID().toString(), mission, method, las);
        try {
