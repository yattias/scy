...
public class SupervisingAgent extends AbstractThreadedAgent implements Callback {
	...
	private static final Tuple query_template = new Tuple("query", Field
			.createWildCardField());

	private static final Tuple response_template = new Tuple("response", Field
			.createWildCardField());
	...
	public SupervisingAgent(Map<String, Object> map) {
		...
		try {
			...
			cbSeq[0] = getTupleSpace().eventRegister(Command.ALL,
					alive_template, this, true);
			cbSeq[1] = getTupleSpace().eventRegister(Command.WRITE,
					query_template, this, true);
			cbSeq[2] = getTupleSpace().eventRegister(Command.WRITE,
	...
	private void responseWritten(Tuple afterTuple) {
		...
		if (!afterTuple.getField(4).getValue().toString().trim().equals(
			...
			logger
					.log(Level.FINEST,
							"Idenfify responses are not handled here.");
		}
	}
	...
	private void aliveWritten(Tuple afterTuple) {
		...
		if (!knownAgents.contains(agentName)) {
			...
			try {
				...
				Tuple identifyResponse = getTupleSpace().waitToTake(t,
						ident_timeout);
