...
public class ELORun extends AbstractRun {
    ...
    public ELORun(String id, String aTitle, 
    		ELOModel aELOModel, TupleID aTupleID, 
    		String actvityStatus, //String eloStatus, 
    		long amountOfWork, long amountOfChange, 
    		long executionTime, long modificationTime) {
		this.id = id;
		this.title = aTitle;
...
/*
    public String getELOStatus() {
        return eloStatus;
    }

    public void setELOStatus(String aStatus) {
        this.eloStatus = aStatus;
    }
*/
    ...
    public String toString() {
    	return new String("ELORun id="+id+", title="+getTitle()+", modelID="+associatedELOModel.getId()+
    			", activityStatus="+activityStatus+//", eloStatus="+eloStatus+
       			", amountOfWork="+amountOfWork+", amountOfChangeWork="+amountOfChangeWork+
       			", executionTime="+executionTime+", changeTime="+changeTime
   			);	
    }
    
    public void updateELOTuple(RunUser user) {
    	try {
    		ProcessGuidanceAgent.getGuidanceSpace().update(myTupleID,
    				new Tuple("elo", user.getId(), user.getMissionRun().getMissionModel().getId(), 
    						getELOModel().getId(), getId(), getTitle(),
    						activityStatus, //eloStatus, 
    						String.valueOf(amountOfWork), String.valueOf(amountOfChangeWork), 
    						String.valueOf(executionTime), String.valueOf(changeTime)));
    ...
    public void handleELORunTuple(RunUser user) {
    	try {
   			Tuple t = ProcessGuidanceAgent.getGuidanceSpace().read(
    				new Tuple("elo", user.getId(), user.getMissionRun().getMissionModel().getId(), 
    						getELOModel().getId(), getId(), getTitle(),
    		...
    		if (t!=null) {
    			...
    			myTupleID = ProcessGuidanceAgent.getGuidanceSpace().write(
        				new Tuple("elo", user.getId(), user.getMissionRun().getMissionModel().getId(), 
        						getELOModel().getId(), getId(), getTitle(),
        						activityStatus, 
        						//eloStatus, 
	...
	public void renameELORun(String elo_uri) {
		try {
            ...
            String title = (String)xPath.evaluate("/elo/metadata/lom/general/title/string", 
	...
	public void handleContentIncreased(RunUser aUser, long aLength, long aTime) {
		...
		if ((getAmountOfChangeWork() > getELOModel().getChangeWorkThreshold())&&
				(getChangeTime() > getELOModel().getChangeTimeThreshold())) {
	...
	public void buildELORun(RunUser aRunUser, String elo_uri) {
		try {
			...
			if (elo_xml!=null) {
	            ...
	            XPath xPath = XPathFactory.newInstance().newXPath();
	            if (title==null) {
	            	title = (String) xPath.evaluate("/elo/metadata/lom/general/title/string", 
							doc, XPathConstants.STRING);
	            }
	            ...
	            relation: for (int i = 0; i < relationNodes.getLength(); i++) { //relation
	            	...
	            	/*
	            	String aValue = (String) xPath.evaluate("//kind/value", relationNode, XPathConstants.STRING);
	            	if ((aValue.equalsIgnoreCase("is_fork_of")) || (aValue.equalsIgnoreCase("is_version_of"))) {
	            		String aURI = (String) xPath.evaluate("//resource/identifier/entry", relationNode, XPathConstants.STRING);
        				...
        				if (aNodeLists.item(j).getNodeName().equalsIgnoreCase("kind")) {
        					...
        					for (int k = 0; k < aKindNodeLists.getLength(); k++) {   
        						if (aKindNodeLists.item(k).getNodeName().equalsIgnoreCase("value")) {
        							...
        							if ((!aKindNodeLists.item(k).getTextContent().equalsIgnoreCase("is_fork_of"))&&
        							(!aKindNodeLists.item(k).getTextContent().equalsIgnoreCase("is_version_of"))) {
        								continue relation;
        							} else {
	            ...
	            String aURI = (String) xPath.evaluate("/elo/metadata/lom/general/identifier/entry", 
						doc, XPathConstants.STRING);
