...
public class ExtractKeywordsDecisionMakerAgent extends AbstractDecisionAgent implements
    IRepositoryAgent {
  ...
  class ContextInformation {
    ...
    public String toString() {
      String webresourcerURL = this.webresourcerELO != null ? this.webresourcerELO.toString()
          : "null";
      return this.user + "|web:" + this.webresourcerStarted + "|map:" + this.scyMapperStarted
             + "|url:" + webresourcerURL + "|" + new Date(this.lastAction).toString() + "|"
  ...
  private void setParameter(Map<String, Object> params) {
    ...
    if (!configuration.containsParameter(IDLE_TIME_INMS)) {
      configuration.setParameter(IDLE_TIME_INMS, 5 * AgentProtocol.MINUTE);
    }
    if (!configuration.containsParameter(TIME_AFTER_USERS_ARE_REMOVED)) {
      configuration.setParameter(TIME_AFTER_USERS_ARE_REMOVED, 120 * AgentProtocol.MINUTE);
    }
  ...
  private void registerToolStartedListener() {
    try {
      this.listenerId = this.getActionSpace().eventRegister(Command.WRITE,
                                                            this.getActionTupleTemplate(), this,
                                                            true);
  ...
  private Tuple getActionTupleTemplate() {
    return new Tuple(AgentProtocol.ACTION, String.class, Long.class, String.class, String.class,
  ...
  private boolean userIsIdleForTooLongTime(long currentTime, ContextInformation contextInformation) {
    ...
    Integer timeAfterThatItIsSaveToRemoveUser = configuration.getParameter(contextInformation.mission,
                                                                           contextInformation.user,
  ...
  private void notifyUser(final long currentTime, final String user,
    ...
    new Thread(new Runnable() {
      ...
      public void run() {
        ...
        if (elo == null) {
          logger.fatal("ELO " + contextInformation.webresourcerELO + " was null");
          return;
        }
      ...
      private IELO getELO(final ContextInformation contextInformation) {
        logger.info("Getting elo: " + contextInformation.webresourcerELO);
  ...
  private boolean userNeedsToBeNotified(long currentTime, ContextInformation contextInformation) {
    ...
    Integer idleTimeInMS = configuration.getParameter(contextInformation.mission,
                                                      contextInformation.user, IDLE_TIME_INMS);
    Integer minimumNumberOfConcepts = configuration.getParameter(contextInformation.mission,
                                                                 contextInformation.user,
  ...
  protected Tuple getIdentifyTuple(String queryId) {
    return new Tuple(AgentProtocol.RESPONSE, queryId, getId(), getName(),
                     AgentProtocol.MESSAGE_IDENTIFY, IDLE_TIME_INMS, MINIMUM_NUMBER_OF_CONCEPTS,
                     TIME_AFTER_USERS_ARE_REMOVED);
  }
