...
public class RegExSentenceSplitter extends DocumentOperatorSpecification {
	...
	public static final String URL_PATTERN = "(http|https)://[-_\\.\\~"
			+ ALPHA_PATTERN + "0-9/=&\\?]+";

	public static final String EMAIL_PATTERN = "[-" + ALPHA_PATTERN
			+ "0-9\\.]+@[-" + ALPHA_PATTERN + "0-9\\.]+";
	...
	public static final String PHRASE_PATTERN = WORD_PATTERN + "([-&]"
			+ WORD_PATTERN + ")*";
	...
	public RegExSentenceSplitter() {
		super();
		this.addParameterType(BASE_TEXT, JavaClasses.STRING, false,
				Features.TEXT);
		this
				.addParameterType(UPDATE, JavaClasses.BOOLEAN, false,
						Boolean.FALSE);
	}
	...
	public Container run(Container inputParameters) {
		...
		if (!Features.getInstance().isFeature(Features.SENTENCES)) {
			Features.getInstance().addFeature(Features.SENTENCES,
					JavaClasses.LIST);
		} else {
			Assert.areSame(Features.getInstance().getType(Features.SENTENCES),
					JavaClasses.LIST);
		}
		...
		if (!document.hasFeature(Features.SENTENCES) || update) {
			...
			String[] s = this.tokenize(text);
			String[] x = this.tokenize(xx);
