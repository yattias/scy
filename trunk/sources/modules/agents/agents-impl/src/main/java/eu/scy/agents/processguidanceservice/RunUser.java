...
public class RunUser extends AbstractRun {	
    ...
    public String toString() {
    	String result = new String("user="+getUserName()+", missionRunID="+currentMissionRun.getCode()+", missionModelID="+
    			currentMissionRun.getMissionModel().getCode()+", mission name="+currentMissionRun.getMissionModel().getName()+
    			", focusedELO="+((focusedELORun==null)?"no":focusedELORun.getTitle())+", guidanceLevel="+generalGuidanceLevel+", openedELORuns: ");	
    	for (int i=0; i<openedELORuns.size(); i++) {
    ...
    public void updateUserTuple() {
    	try {
    		ProcessGuidanceAgent.getGuidanceSpace().update(myTupleID,
    				new Tuple("user", getId(), generalGuidanceLevel));
    ...
    public void buildRunUser() {
    	try {
    		...
    		Tuple aUserTuple = ProcessGuidanceAgent.getGuidanceSpace().read(
    				new Tuple("user", getId(), Field.createWildCardField()));
    		if (aUserTuple==null) {
    			//a new user
	...
	private Tuple createAgendaNotification(String notificationId, String user,
	...
	private Tuple createMessageNotification(String notificationId, String user,
	...
	public void provideGuidanceAfterComplete() {
		...
		try {
			...
			Tuple completeNotificationTuple = createAgendaNotification(completeNotificationId,
					this.getId(), 
					getFocusedELORun().getTitle(), 
					getFocusedELORun().getActivityStatus(), 
					String.valueOf(getFocusedELORun().getFinishedTime()), 
					getFocusedELORun().getId());							
			...
			if (aTitle!=null) {				
				messageNotificationTuple = createMessageNotification(messageNotificationId,
						this.getId(), 
						"You have completed the activity: \""+getFocusedELORun().getTitle()+"\". The recommanded next activity is \""+aTitle+"\".", 
						String.valueOf(getFocusedELORun().getFinishedTime()));
			} else {
				messageNotificationTuple = createMessageNotification(messageNotificationId,
						this.getId(),
