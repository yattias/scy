...
public class MissionRun extends AbstractRun {
    ...
    public String toString() {
    	String result;
    	result = new String("missionRunID="+id+", user name="+user.getId()+", mission name="+
    			associatedMissionModel.getName()+", guidance level="+missionGuidanceLevel+", ELORuns: ");	
    	for (int i=0; i<eloRuns.size(); i++) {
    ...
    public void updateMissionRunTuple() {
    	try {
    		ProcessGuidanceAgent.getGuidanceSpace().update(myTupleID,
    				new Tuple("mission", user.getId(), associatedMissionModel.getId(), id, 
							user.getGeneralGuidanceLevel() /*, start_time*/));
    ...
    public void buildMissionRun(){
    	try {
    		...
    		Tuple aMissionRunTuple = ProcessGuidanceAgent.getGuidanceSpace().read(
					new Tuple("mission", user.getId(), associatedMissionModel.getId(), Field.createWildCardField()));
			if (aMissionRunTuple==null) {
				...
				Tuple[] allELOsInMission = ProcessGuidanceAgent.getGuidanceSpace().readAll(
						new Tuple("elo", user.getId(), associatedMissionModel.getId(), Field.createWildCardField()));
				...
				for (Tuple t : allELOsInMission) {
					...
					ELORun anELORun = new ELORun(eloRunID, eloRunTitle, 
									aELOModel, t.getTupleID(),
									(String)t.getField(6).getValue(),
									//(String)t.getField(7).getValue(),
									Long.parseLong((String)t.getField(7).getValue()),
									Long.parseLong((String)t.getField(8).getValue()),
									Long.parseLong((String)t.getField(9).getValue()),
									Long.parseLong((String)t.getField(10).getValue()));
					addELORun(anELORun);
				}
			}
		} catch (TupleSpaceException e) {
