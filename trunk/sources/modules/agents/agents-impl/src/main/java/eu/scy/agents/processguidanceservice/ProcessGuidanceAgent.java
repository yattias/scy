...
public class ProcessGuidanceAgent implements Callback {
	...
	public static TupleSpace getGuidanceSpace() {
		if (guidanceSpace == null) {
			try {
				guidanceSpace = new TupleSpace(User.getDefaultUser(), 
						"scy.collide.info", 2525, false, false, "guidance");;
			} catch (TupleSpaceException e) {
				e.printStackTrace();
			}
		}
	...
	public static TupleSpace getActionSpace() {
		if (actionSpace == null) {
			try {
				actionSpace = new TupleSpace(User.getDefaultUser(), 
						"scy.collide.info", 2525, false, false, AgentProtocol.ACTION_SPACE_NAME);;
			} catch (TupleSpaceException e) {
				e.printStackTrace();
			}
		}
	...
	public static TupleSpace getCommandSpace() {
		if (commandSpace == null) {
			try {
				commandSpace = new TupleSpace(User.getDefaultUser(), 
						"scy.collide.info", 2525, false, false, AgentProtocol.COMMAND_SPACE_NAME);;
			} catch (TupleSpaceException e) {
				e.printStackTrace();
			}
		}
    ...
    private void handleTextInserted(Action action) {
    	...
    	if (aELORun!=null) {
    		...
    		aRunUser.getActionHistory().addAction(
    				new UserAction(UserAction.ACTION_TEXT_INSERTED, aELORun, action.getTimeInMillis()));
    	}
    }    
    ...
    private void handleTextDeleted(Action action) {
    	...
    	if (aELORun!=null) {
    		...
    		aRunUser.getActionHistory().addAction(
    				new UserAction(UserAction.ACTION_TEXT_DELETED, aELORun, action.getTimeInMillis()));
    	}
    }
    ...
    private void handleToolStarted(Action action) {
    	...
    	if (aELORun==null) {
    		...
    		if (aELORun.getELOModel()!=null){
        		...
        		aRunUser.getActionHistory().addAction(
        				new UserAction(UserAction.ACTION_TOOL_STARTED, aELORun, action.getTimeInMillis()));
    		} else {
    ...
    private void handleToolOpened(Action action) {
    	...
    	if (aELORun!=null) {
    		...
    		aRunUser.getActionHistory().addAction(
    				new UserAction(UserAction.ACTION_TOOL_OPENED, aELORun, action.getTimeInMillis()));
    	} else {
    ...
    private void handleToolGotFocus(Action action) {
    	...
    	if (aELORun!=null) {
    		...
    		aRunUser.getActionHistory().addAction(
    				new UserAction(UserAction.ACTION_TOOL_GOT_FOCUS, 
    						aELORun, action.getTimeInMillis()));
    	}
    }
    ...
    private void handleELOSaved(Action action) {
    	...
    	if (aELORun!=null) {
			...
			aRunUser.getActionHistory().addAction(
    				new UserAction(UserAction.ACTION_ELO_SAVED, aELORun, action.getTimeInMillis()));
    	}    	
    }
    ...
    private void handleToolComplete(Action action) {
    	...
    	if (aELORun!=null) { //a planned elo
    		aELORun.handleCompleteEvent(aRunUser, action.getTimeInMillis());
    		aRunUser.getActionHistory().addAction(
    				new UserAction(UserAction.ACTION_TOOL_COMPLETE, aELORun, action.getTimeInMillis()));
       	}        		
    }
    ...
    private void handleToolLostFocus(Action action) {
    	...
    	ELORun aELORun = aRunUser.findOpenedELORunByURI(elo_uri);
    	if ((aELORun!=null)&&
    			(elo_uri.equalsIgnoreCase(aRunUser.getFocusedELORun().getId()))) {
    		aRunUser.setFocusedELORun(null);  
    		aRunUser.getActionHistory().addAction(
    				new UserAction(UserAction.ACTION_TOOL_LOST_FOCUS, aELORun, action.getTimeInMillis()));
    	}
    }
    ...
    private void handleToolClosed(Action action) {
		...
		aRunUser.getActionHistory().addAction(
				new UserAction(UserAction.ACTION_TOOL_CLOSED, aELORun, action.getTimeInMillis()));
    }    
    ...
    private void handleToolQuit(Action action) {
		...
		aRunUser.getActionHistory().addAction(
				new UserAction(UserAction.ACTION_TOOL_QUIT, aELORun, action.getTimeInMillis()));
    }
