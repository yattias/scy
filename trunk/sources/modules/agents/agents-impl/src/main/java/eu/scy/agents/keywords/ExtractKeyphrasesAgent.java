...
/**
 * (ExtractKeyphrases:String, query:String, <QueryId>:String, <Text>:String)-> (ExtractKeyphrases:String, response:String,
 * <QueryId>:String,
 * <Keywords>:String)->
 *
...
public class ExtractKeyphrasesAgent extends AbstractThreadedAgent {
    ...
    public ExtractKeyphrasesAgent(Map<String, Object> params) {
        ...
        activationTuple = new Tuple(EXTRACT_KEYPHRASES, AgentProtocol.QUERY, String.class,
    ...
    private CorpusModel getCorpusModel(String mission, String language) {
        CorpusModel cpModel = modelStorage.get(mission, language,
                KeywordWorkflowConstants.KEYPHRASE_MODEL);
        return cpModel;
    }
    ...
    private Properties initWorkflowProperties() {
        ...
        workflowProperties.setProperty(WikiParameters.DIRECTORY, "mission1_texts/English");
         ...
         */
        workflowProperties.setProperty(WikiParameters.FILE_TYPE, WikiValues.FILE_TYPE_TXT);

        /** minimum length that a token must have */
        workflowProperties.setProperty(WikiParameters.MIN_TOKEN_LENGTH, Integer.valueOf(1).toString());
        ...
        workflowProperties.setProperty(WikiParameters.MIN_NGRAM_TOKENS, Integer.valueOf(2).toString());
        /** maximum number of tokens that a n-gram can have */
        workflowProperties.setProperty(WikiParameters.MAX_NGRAM_TOKENS, Integer.valueOf(5).toString());
        /** minimum number of appearances in the document a n-gram must have */
        workflowProperties.setProperty(WikiParameters.MIN_FREQUENCY, Integer.valueOf(2).toString());
        /**
        ...
        workflowProperties.setProperty(WikiParameters.NORMALIZE_INFORMATIVENESS,
        ...
        workflowProperties.setProperty(WikiParameters.PHRASENESS_WEIGHT, Float.valueOf(0.5f).toString());
    ...
    private Set<String> extractKeyphrases(String text, String mission, String language) {
        CorpusModel cpModel = getCorpusModel(mission, language);
        if ( cpModel == null ) {
            logger.warn("Corpus Model for mission " +  mission +"/" + language +" for keyphrase extraction is not present");
            return Collections.emptySet();
        }

        Document document = Utilities.convertTextToDocument(text);
        Operator extractKeyphrasesOperator = new ExtractKeyphrasesWorkflow(initWorkflowProperties()).getOperator("Main");
        extractKeyphrasesOperator.setInputParameter(ObjectIdentifiers.DOCUMENT, document);
        extractKeyphrasesOperator.setInputParameter(KeywordWorkflowConstants.KEYPHRASE_MODEL, cpModel);

        Container output = extractKeyphrasesOperator.run();

        Document doc = (Document) output.getObject(ObjectIdentifiers.DOCUMENT);

        Map<String, Float> keyphraseMap = (HashMap<String, Float>) doc.getFeature(WikiFeatures.NGRAM_FREQUENCIES);
