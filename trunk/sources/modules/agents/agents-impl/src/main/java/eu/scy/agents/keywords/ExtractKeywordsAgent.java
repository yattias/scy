...
public class ExtractKeywordsAgent extends AbstractRequestAgent {
  ...
  private Set<String> mergeKeywords(Set<String> tfIdfKeywords, Set<String> keyPhrases,
  ...
  private Set<String> callKeywordsAgent(String agent, String text) {
    ...
    try {
      ...
      Tuple response = getCommandSpace().waitToTake(new Tuple(agent, AgentProtocol.RESPONSE,
  ...
  public void call(Command command, int seq, Tuple tuple, Tuple beforeTuple) {
    if (listenerId != seq) {
      ...
      Set<String> tfIdfKeywords = callKeywordsAgent(ExtractTfIdfKeywordsAgent.EXTRACT_TFIDF_KEYWORDS,
                                                    text);
      Set<String> keyPhrases = callKeywordsAgent(ExtractKeyphrasesAgent.EXTRACT_KEYPHRASES, text);
      Set<String> topicKeywords = callKeywordsAgent(ExtractTopicModelKeywordsAgent.EXTRACT_TOPIC_MODEL_KEYWORDS,
                                                    text);
      Set<String> ontologyKeywords = callKeywordsAgent(OntologyKeywordsAgent.EXTRACT_ONTOLOGY_KEYWORDS,
                                                       text);

      Set<String> mergedKeywords = mergeKeywords(tfIdfKeywords, keyPhrases, topicKeywords,
                                                 ontologyKeywords);
