...
:- module(reference_model_specification_ic,
	  [ reference_model_parse/2,		% XMLorFile x Options	// deprecated
	    ...
	    reference_model_node/2,   		% RM -> Node
	    reference_model_node/3,		% RM -> Node x Properties
	    reference_model_node_properties/3,	% RM -> Node x Properties
	    reference_model_node_property/3,	% RM -> Node x Property
	    reference_model_node_propchk/3,	% RM -> Node x Property

	    reference_model_link/2,   		% RM -> Link
	    reference_model_link/3,		% RM -> Link x Properties
	    reference_model_link_properties/3,	% RM -> Link x Properties
	    reference_model_link_property/3,	% RM -> Link x Property
	    reference_model_link_propchk/3,	% RM -> Link x Property

	    reference_model_edge/3,		% RM -> Tail x Head
	    reference_model_edge/4,		% RM -> Tail x Head x Properties
	    reference_model_edge_properties/4,	% RM -> Tail x Head x Properties
	    reference_model_edge_property/4,	% RM -> Tail x Head x Properties
	    reference_model_edge_propchk/4,	% RM -> Tail x Head x Properties

	    reference_model_t_edge/3,		% RM -> Tail x Head
	    reference_model_t_edge/4,		% RM -> Tail x Head x Properties
	    reference_model_t_edge_properties/4,% RM -> Tail x Head x Properties
	    reference_model_t_edge_property/4,	% RM -> Tail x Head x Properties
	    reference_model_t_edge_propchk/4,	% RM -> Tail x Head x Properties

	    reference_model_p_edge/3,		% RM -> Tail x Head
	    reference_model_p_edge/4,		% RM -> Tail x Head x Properties
	    reference_model_p_edge_properties/4,% RM -> Tail x Head x Properties
	    reference_model_p_edge_property/4,	% RM -> Tail x Head x Properties
	    reference_model_p_edge_propchk/4	% RM -> Tail x Head x Properties
	  ]).
...
reference_model_create(RM, Properties) :-
	reference_model_delete(RM),
	assert(reference_model_properties(RM,Properties)).
...
reference_model_property(RM, Prop) :-
	reference_model_properties(RM, Properties),
...
reference_model_node(RM, Node, Properties) :-
	format('**** reference_model_node/3 deprecated ****~n', []),
	node(RM, Node, Properties).

reference_model_node_properties(RM, Node, Properties) :-
	node(RM, Node, Properties).

reference_model_node_property(RM, Node, Prop) :-
	node(RM, Node, Properties),
	member(Prop, Properties).

reference_model_node_propchk(RM, Node, Prop) :-
	node(RM, Node, Properties),
	memberchk(Prop, Properties).
...
reference_model_link(RM, Link, Properties) :-
	format('**** reference_model_link/3 deprecated ****~n', []),
	link(RM, Link, Properties).

reference_model_link_properties(RM, Link, Properties) :-
	link(RM, Link, Properties).

reference_model_link_property(RM, Link, Prop) :-
	link(RM, Link, Properties),
	member(Prop, Properties).

reference_model_link_propchk(RM, Link, Prop) :-
	link(RM, Link, Properties),
	memberchk(Prop, Properties).
...
reference_model_edge(RM, From, To, Properties) :-
	format('**** reference_model_edge/3 deprecated ****~n', []),
	edge(RM, From, To, Properties).

reference_model_edge_properties(RM, From, To, Properties) :-
	edge(RM, From, To, Properties).

reference_model_edge_property(RM, From, To, Prop) :-
	edge(RM, From, To, Properties),
	member(Prop, Properties).

reference_model_edge_propchk(RM, From, To, Prop) :-
	edge(RM, From, To, Properties),
	memberchk(Prop, Properties).
...
reference_model_t_edge(RM, From, To, Properties) :-
	format('**** reference_model_t_edge/3 deprecated ****~n', []),
	t_edge(RM, From, To, Properties).

reference_model_t_edge_properties(RM, From, To, Properties) :-
	t_edge(RM, From, To, Properties).

reference_model_t_edge_property(RM, From, To, Prop) :-
	t_edge(RM, From, To, Properties),
	member(Prop, Properties).

reference_model_t_edge_propchk(RM, From, To, Prop) :-
	t_edge(RM, From, To, Properties),
	memberchk(Prop, Properties).
...
reference_model_p_edge(RM, From, To, Properties) :-
	format('**** reference_model_p_edge/3 deprecated ****~n', []),
	p_edge(RM, From, To, Properties).

reference_model_p_edge_properties(RM, From, To, Properties) :-
	p_edge(RM, From, To, Properties).

reference_model_p_edge_property(RM, From, To, Prop) :-
	p_edge(RM, From, To, Properties),
	member(Prop, Properties).

reference_model_p_edge_propchk(RM, From, To, Prop) :-
	p_edge(RM, From, To, Properties),
	memberchk(Prop, Properties).
...
parse_t(reference_model, As, C, RM, _) :- !,
	xml_avs_to_terms(As, Properties),
	assert(reference_model_properties(RM,Properties)),
	option(notation(Rep), Properties, sdm),
	parse(C, RM, Rep).
...
:- dynamic
	...
	reference_model_properties/2,	% RM x Properties
	node/3,				% RM x Term x Properties
	link/3,				% RM x Term x Properties
	edge/4,				% RM x Tail x Head x Properties
	t_edge/4,			% RM x Tail x Head x Properties
	p_edge/4.			% RM x Tail x Head x Properties
