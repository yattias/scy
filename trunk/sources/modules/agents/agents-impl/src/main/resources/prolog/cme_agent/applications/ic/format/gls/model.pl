...
:- module(model_gls_ic,
	  [ gls/1,			% <-> GLS
	    ...
	    gls_node_delete/2,
	    gls_node_replace_by/3,	% GLS x Node x Node
...
%%	gls_session(+G:gls_handle, -Session:atom) is semidet.
%
%	Succeeds if Session is the session in which GLS was created.
%	This can fail if no session information is available.

gls_session(G, Session) :-
	gls_propchk(G, session(Session)), !.
gls_session(G, Session) :-
	gls_propchk(G, notation(Notation)),
	(   Notation == cmap
	...
	gls_add_property(G, session(Session)).
...
cmap_node_to_gls([Prop|Props], Node, Module) :-
	...
	->  true
	;   format('gls(model): **** cmap node property ~q ****~n', [Prop])
	),
...
cmap_node_prop_to_gls(text_colour(RGB), Node, Module) :-
	add_node_visual(Node, text_colour(RGB), Module).
cmap_node_prop_to_gls(shape_colour(RGB), Node, Module) :-
	add_node_visual(Node, shape_colour(RGB), Module).
cmap_node_prop_to_gls(shape(Shape), Node, Module) :-
	add_node_visual(Node, shape(Shape), Module).
...
cmap_edge_prop_to_gls(direction(Dir), Edge, Module) :-
	assert(Module:edge_direction(Edge,Dir)).
...
cmap_edge_prop_to_gls(shape(Shape), Edge, Module) :-
	add_edge_visual(Edge, shape(Shape), Module).
cmap_edge_prop_to_gls(colour(RGB), Edge, Module) :-
	add_edge_visual(Edge, colour(RGB), Module).
...
sdm_node_props_to_gls([Prop|Props], Node, Module) :-
	...
	->  true
	;   format('gls(model):  sdm_node unknown property ~w~n', [Prop])
	),
	sdm_node_props_to_gls(Props, Node, Module).
...
gls_node_delete(G, Node) :-
	gls_module(G, Module),
	retractall(Module:node(Node)),
	retractall(Module:node_label(Node,_)),
	retractall(Module:node_term(Node,_)),
	retractall(Module:node_term_status(Node,_)),
	retractall(Module:node_type(Node,_)),
	retractall(Module:node_visuals(Node,_)),
	retractall(Module:node_properties(Node,_)),
	retractall(Module:edge_tail(_,Node)),
	retractall(Module:edge_head(_,Node)).

gls_node_replace_by(G, OldNode, NewNode) :-
	gls_module(G, Module),
	forall(retract(Module:edge_tail(Edge, OldNode)),
	       assert(Module:edge_tail(Edge,NewNode))),
	forall(retract(Module:edge_head(Edge, OldNode)),
	       assert(Module:edge_head(Edge,NewNode))).
