 ...
 * <br>
 * 
 * There are some conventions regarding the agents which can be managed using this manager: - All manageable agents must be an implementation of {@link IThreadedAgent}.<br>
 * <br>
 * 
 * - All manageable agents must implement a constructor like {@code MyAgent(Map<String, Object> map)} This maps is usually used to set some properties. If you do not need properties, you can ignore the map exept for the entry <i>"id"</i> which contains the uniqueID ({@link VMID} as String) of the
 * single instances.<br>
 * - The whole inter-agent-communication is handled using the SQLSpaces (http://weinbrenner.collide.info/sqlspaces-site/) and follows following convention:<br/> {@code ("query"|"response"|"agentCommand"), queryID [String], agentID [String], agentName [String], serviceName [String], ....(Payload)}
 * 
...
public class AgentManager implements Callback {
    ...
    /**
     * This constructor is used to start an agent manager. Please make sure that only one instance is running on one SQLSpace. We intentionally used no Singleton Pattern or Lazy creation to provide the possibility to have several managers running parallel, but you have to know what you are doing.
     * 
     * @param host
     ...
     * 
     * After sent the tuple, the agent is taken from the list of actual agents ( {@code agentIdMap}) and a reference is stored in the {@code oldAgents} -Map.<br>
     * This is to make sure that if an alive-tuple with the id of an already killed agent is received (via callback) this agent can be referenced and killed the <i>"hard way"</i> (via {@code Thread.stop()})
     * 
     * @param agent
    ...
    /**
     * This methods sends a stop-Tuple to the passed agentId({@link VMID} as String). The agent itself have to make sure that it is stopped correctly.
     * 
     * After sent the tuple, the agent is taken from the list of actual agents ( {@code agentIdMap} and a reference is stored in the {@code oldAgents}-Map This is to make sure that if an alive-tuple with the id of an already killed agent is received (via callback) this agent can be referenced and
    ...
    /**
     * This method is used to start an agent using this manager. To start this {@link IThreadedAgent} you have to pass the (fully qualified) name (e.g. eu.scy.agents.impl.ThreadedAgentMock) which can be accessed using the classpath. You can pass a {@link Map} with properties to the agent. If you
     * pass {@code null} instead, a new HashMap with only the instance ID of this agent will be created.
     * 
     * @param name
     *            The (fully qualified, classpath accessible) Name of the new Agent
     * @param params
     *            A {@link Map} with properties. Can be {@code null}
     * @return The new (just started) {@link IThreadedAgent}
     * @throws AgentLifecycleException
     *             If something went wrong during the creation of the new agent (e.g. in the process of reflection) this Exception is thrown
     */
    ...
    /**
     * This Method uses the "hard-way" to stop an agent. If an agent is still alive although it has been forced to stop the Thread in which this agent lives will be stopped by Thread.stop()
     * 
     * @param id
     *            The uniqueID ({@link VMID} as String) of the agent
     * @throws AgentLifecycleException
     *             If something went wrong during the killing of this agent this exception is thrown.
     */
        ...
        if (AgentProtocol.ALIVE_TUPLE_TEMPLATE.matches(afterTuple)) {
            if ((Command.WRITE == command) || (Command.UPDATE == command)) {
                ...
                if (oldAgents.containsKey(agentId)) {
                    logger.log(Level.WARNING, "Agent " + agentId + " should be stopped, but is still running. Trying to kill it the hard way ...");
    ...
    /*
     * This Method is used internally to restart an agent which should be alive but isn't. We use the kill method to stop this agent the "hard way" to make sure that (a potential) zomie-instance is shut down. After that we start an agent using his parameter.
     */
