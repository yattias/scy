...
/**
 * Extern-Link action for similarity document library portlet. Add extern links
 * from a chosen start resource to an entered url. It is fired after pressing
 * the taglib ui add_link. If the user chose callByRefence or let the radio
 * button empty an new bookmark entry will create. A new link from viewed
 * resource to created where created and a new link from created bookmark. If
 * user chose callByValue the content from entered url will be save from stream
 * as new webcontent and also links between the resource would be create.
 * 
 * @author daniel
 * 
 */
public class LinkAction extends PortletAction {
	...
	public void processAction(ActionMapping mapping, ActionForm form, PortletConfig portletConfig, ActionRequest req, ActionResponse res) throws Exception {
		...
		try {
			entry = DLFileEntryLocalServiceUtil.getDLFileEntry(entryId);
			ServiceContext serviceContext = ServiceContextFactory.getInstance(BookmarksEntry.class.getName(), req);
			if (cmd == null) {
				req.setAttribute(WebKeys.DOCUMENT_LIBRARY_FILE_ENTRY, entry);
				req.setAttribute("redirect", redirect);

				// set Forward
				setForward(req, "portlet.document_library.view_link");
				return;
				...
				String classNameIdFile = String.valueOf(ClassNameLocalServiceUtil.getClassNameId(DLFileEntry.class.getName()));
				// add link for blog
				util.addLink(entryId.toString(), linkedResourceId.toString(), classNameIdBookmark, serviceContext);
				// add link for new bookmark
				util.addLink(linkedResourceId.toString(), entryId.toString(), classNameIdFile, serviceContext);
	...
	public ActionForward render(ActionMapping mapping, ActionForm form, PortletConfig portletConfig, RenderRequest renderRequest, RenderResponse renderResponse)
		...
		return mapping.findForward(getForward(renderRequest, "portlet.document_library.view_link"));
	}
}
