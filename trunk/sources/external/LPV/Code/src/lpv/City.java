...
class City {
	...
	Vector<Integer> actorsid; // actorsid.get(3) is the external name (id) of user with internal #3
	 double zeroX, zeroY; // coordinates of the external zero point (in local base)
	private double zeroXdelta = 0; //zeroX + zeroXdelta = position in hard space 
	 double scale, scaleY=0.5;
	...
	public void addProcessAndOrganize(int from, int to, int name, int who, long time){
	...
	*/	
		if(pom.getFrom()!=getRoot()) { 
			pom.to.setHardCenterY(pom.from.getHardCenterY());
		} else {
			pom.to.setHardCenterY(rootIndexToY(getRoot().usedByProcesses.size()-1));
		}
		pom.to.setHardCenterX(timeToX(pom.to.getTime()));
		if(timeOrderQueue != null) {
			timeOrderQueue.add(pom.to);
		}else
		{
		...
		if(pom.getTime()>maxtime) maxtime = pom.getTime();
		maxwidth = getRoot().usedByProcesses.size();
	...
	private long rootIndexToY(int i) {
		 return (long)(-(i+1)*heightSpacing*productHeight);
		
	}

	private long timeToX(long time) {
		return (long) ((time-zerotime)/timeScale);
	}
	...
	*/	
	...
	public void addProcessAndOrganize(String old_uri, String elo_uri, String tool,
			String user, long time) {
		// TODO Auto-generated method stub
		addProcessAndOrganize(productDict.indexOf(old_uri), productDict.indexOf(elo_uri), addToDict(toolDict, tool), addToDict(userDict, user), time);
	...
	*/
	...
	public void organize(){
		Stack<ELOProcess> st = new Stack<ELOProcess>(); // TUBAS TUBAS STACK
		Collections.sort(getRoot().usedByProcesses,Collections.reverseOrder());
		st.addAll(getRoot().usedByProcesses);
		Collections.reverse(getRoot().usedByProcesses);
		
		lpv.ELOProcess dummy;
		maxwidth = 0;
		...
		while(st.size()>0){
			...
			if(dummy.isOriginal()){
				dummy.getTo().setHardCenterPosition(timeToX(dummy.getTime()),rootIndexToY(maxwidth++));
			}
			else{
				dummy.getTo().setHardCenterPosition(timeToX(dummy.getTime()), dummy.getFrom().getCenterY());
			}
			...
			Collections.reverse(dummy.getTo().usedByProcesses);
		}
	...
	//	height=Math.max(height,(maxwdth*2+2)*productHeight)+200;
		if(getRoot().usedByProcesses.size()>0){
			...
			//setZeroY(-cityToScreenCoordY(getRoot().usedByProcesses.get(0).getTo().getCornerY()));
			setZeroY(-height+(int)heightSpacing*productHeight);
			setZeroX(-15);
		}
	...
	/*	for (ArrayList<Double> jump : jumps) {
		...
		for (int i=0;i<jumps.size();i++) {
			ArrayList<Long> jump = jumps.get(i);
	...
	*/	
 	 ...
 	 private void drawLabels(Graphics2D gr2) {
        ...
        Font font = new Font("SansSerif", Font.PLAIN, 17);
 		for(int i=0;i<max;i++) {
			double yCenter = cityToScreenCoordY(rootIndexToY(i));
			
			String label = toolDict.get(getRoot().usedByProcesses.get(i).getName());
	        gr2.setFont(font);
