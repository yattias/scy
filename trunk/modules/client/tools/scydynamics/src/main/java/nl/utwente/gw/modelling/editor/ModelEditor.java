 ...
 */
public class ModelEditor extends JPanel implements AdjustmentListener, ActionListener {
...
//			userMessage = JTools.getAppResourceString("editorMsgReservedWord",name);
		...
		for (JdObject o : aModel.getObjects().values()) {
			...
			case JdFigure.STOCK:
				...
				if (!n.isGraphExpr()) // do not check GRAPH(...) expressions
					b3 = checkExpr(n.getLabel(), n.getExpr(), aModel
							.getLinkedVariablesTo(n, false), true);
				o.setSpecified(b3);
				if (!b3 && !b)
					b = false;
				break;
	...
	public void showSpecDialog(JdFigure figure, java.awt.Point position) {
		...
		VariableDialog vdialog = new VariableDialog(frame, position, figure, aModel);
...
//		String noSpecMsg = JTools.getAppResourceString("EditorSpecMsgSelNone");
...
//				Vector<String> vl = JTools.sortStringVector(aModel.getLinkedVariablesTo(fNode,false));
//				h.put("linked", vl);
//				h.put("linkedColors", aModel.getObjectsLabelColors(vl));
//				if (o.isAux()) {
//					h.put("linkedRel", JTools.sortStringVector(aModel.getLinkedVariablesTo(fNode,true)));
//				}
	...
	public void setFigureProperties(String oldName, Hashtable<String, Object> h) {
		...
		case JdFigure.RELATION:
			...
			if (h.containsKey("relType")) {
				if (r.getRelationType() != ((Integer) h.get("relType"))
						.intValue()) {
					bModelChanged = true;
				}
			}
		...
		if (h.containsKey("relations")) {
			aModel.updateNodeRelations(((JdNode) o).getLabel(), (Hashtable) h
					.get("relations"));
			// updateSpecDialog(false);
		} else if (bChangeRel) {
			aModel.changeNodeRelations(((JdNode) o).getLabel(), ((Integer) h
					.get("exprType")).intValue());
			updateSpecDialog(false);
		}
		if (bModelChanged)
			setModelChanged();
