...
import com.scy.scyDataTool.controller.ScyMath;
import com.scy.scyDataTool.pdsELO.GraphVisualization;
import java.util.ArrayList;

import com.scy.scyDataTool.pdsELO.ProcessedData;
...
public class Dataset implements Cloneable{
    ...
    public ProcessedDatasetELO toELO(Locale locale){
        ...
        headers.add(header);
        // rows
        List<com.scy.scyDataTool.pdsELO.Data> listIgnoredData = new LinkedList<com.scy.scyDataTool.pdsELO.Data>();
        ...
        for (int i=0; i<nbR; i++){
            ...
            for (int j=0; j<nbC; j++){
                ...
                if (getData(i, j) != null && getData(i, j).isIgnoredData())
                    listIgnoredData.add(new com.scy.scyDataTool.pdsELO.Data(Integer.toString(i), Integer.toString(j)));
            }
        ...
        // PROCESSED DATASET
        com.scy.scyDataTool.pdsELO.IgnoredData iData =new com.scy.scyDataTool.pdsELO.IgnoredData(listIgnoredData);
        List<com.scy.scyDataTool.pdsELO.Operation> listOperations = new LinkedList<com.scy.scyDataTool.pdsELO.Operation>();
        ...
        for (int i=0; i<nbop; i++){
            ...
            com.scy.scyDataTool.pdsELO.Operation operation  = new com.scy.scyDataTool.pdsELO.Operation(myOp.isOnCol(), myOp.getName(), myOp.getName(), "double", myOp.getTypeOperation().getCodeName(), references, results, allParam);
            listOperations.add(operation);
        }
        List<com.scy.scyDataTool.pdsELO.Visualization> listVisualizations = new LinkedList<com.scy.scyDataTool.pdsELO.Visualization>();
        ...
        for (int i=0; i<nbVis; i++){
            ...
            com.scy.scyDataTool.pdsELO.Visualization vis = null;
            if (type.getCode() == DataConstants.VIS_GRAPH){
                Graph g = (Graph)myVis ;
                ParamGraph pg = g.getParamGraph() ;
                List<com.scy.scyDataTool.pdsELO.FunctionModel> listFunction = getListFunctionModel(g.getListFunctionModel());
                ...
                vis = new com.scy.scyDataTool.pdsELO.PieVisualization(DataConstants.TYPE_VIS_PIE, myVis.getName(), myVis.isOnCol, myVis.getTabNo()[0]);
            }else if (type.getCode() == DataConstants.VIS_BAR){
                vis = new com.scy.scyDataTool.pdsELO.BarVisualization(DataConstants.TYPE_VIS_BAR, myVis.getName(), myVis.isOnCol, myVis.getTabNo()[0]);
            }
            if(vis != null)
    ...
    private List<com.scy.scyDataTool.pdsELO.FunctionModel> getListFunctionModel(ArrayList<FunctionModel> listFm){
        List<com.scy.scyDataTool.pdsELO.FunctionModel> list = null;
        if (listFm != null){
            int nb = listFm.size();
            list = new LinkedList();
            for (int i=0; i<nb; i++){
                FunctionModel fm = listFm.get(i);
                com.scy.scyDataTool.pdsELO.FunctionModel f = new com.scy.scyDataTool.pdsELO.FunctionModel(fm.getDescription(), fm.getColor().getRed(), fm.getColor().getGreen(), fm.getColor().getBlue());
                list.add(f);
            }
        }
        return list;
