...
public class ActionTupleTransformer {
    ...
    private IAction actionAsPojo;

    private Tuple actionAsTuple;

    public ActionTupleTransformer(IAction actionAsPojo) {
        this.actionAsPojo = actionAsPojo;
    }

    public ActionTupleTransformer(Tuple actionAsTuple) {
        this.actionAsTuple = actionAsTuple;
    }

    public IAction getActionAsPojo() {
        if (actionAsPojo != null) {
            return actionAsPojo;
        } else {
            actionAsPojo = new Action((String) actionAsTuple.getField(1).getValue());
    ...
    public Tuple getActionAsTuple() {
        if (actionAsTuple != null) {
            return actionAsTuple;
        } else {
            actionAsTuple = new Tuple();
