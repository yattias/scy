...
public class ServerResponseService {
    ...
    private static final Logger log = Logger.getLogger(ServerResponseService.class.getName());
    private static final LoginManager loginManager = LoginManager.getInstance();

    /** Creates a new instance of ServerResponseService */
    ...
    public String challengeRespsonse(JSONObject jsonData) {
        ...
        try {
            ...
            cr = jsonData.optString("cr", null);
                if (cr != null) {
                    loginManager.getChallenge(username).setCr(cr);
                }
                String sr = createSR(username);
                return sr;
