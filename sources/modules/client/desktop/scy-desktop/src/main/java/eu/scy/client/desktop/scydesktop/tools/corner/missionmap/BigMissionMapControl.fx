...
public class BigMissionMapControl extends CustomNode {
   ...
   public function teleportToLas(lasId: String, fading: Boolean): Void {
      XFX.deferActionAndWait(function(): Void {
         scyDesktop.bigMissionMapControl.showBigMissionMap(fading);
      });

      XFX.runActionAfter(function(): Void {
         scyDesktop.missionModelFX.setSelectedLas(lasId);
         scyDesktop.bigMissionMapControl.hideBigMissionMap(fading);
      }, 1s);
   }

   public function showBigMissionMap(): Void {
      showBigMissionMap(false);
   }

   public function showBigMissionMap(fading: Boolean): Void {
      if (not bigMissionMapVisible) {
         ...
         ModalDialogLayer.addModalDialog(missionMapWindow, true, fading, true, 0);
   ...
   public function hideBigMissionMap(): Void {
      hideBigMissionMap(false);
   }

   public function hideBigMissionMap(fading: Boolean): Void {
      if (bigMissionMapVisible) {
         bigMissionMapVisible = false;
         //            ModalDialogLayer.removeModalDialog(missionMapWindow, fading);
         ModalDialogLayer.removeModalDialog(missionMapWindow);
         if (missionMapInstructionAvailable) {
            hideInstructionWindow();
            //                ModalDialogLayer.removeModalDialog(instructionWindow, fading, true);
   ...
   function openElo(scyElo: ScyElo, las: LasFX): Void {
      missionModel.setSelectedLas(las);
