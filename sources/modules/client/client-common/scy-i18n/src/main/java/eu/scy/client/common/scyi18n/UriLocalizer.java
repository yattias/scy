...
public class UriLocalizer
{
   ...
   public String localizePath(String path)
   {
      ...
      // boolean match = path.matches(inPathPattern);
      if (languageInPathMatch.find())
      {
         int startMatchPos = languageInPathMatch.start();
         int endMatchPos = languageInPathMatch.end();
         localizedPath = path.substring(0, startMatchPos) + localeLanguage + "/"
                  + path.substring(endMatchPos);
      }
      else
      {
         ...
         if (languageInNameMatch.find())
         {
            ...
            localizedPath = path.substring(0, startMatchPos) + localeLanguage + "."
                     + path.substring(startMatchPos + localeLanguage.length() + 1);
         }
      }
