...
public class MathToolController {
	...
	public void addCanvas(String type, ShapeCanvas shapeCanvas) {
		...
		shapeCanvas.setDropTarget(new DropTarget(shapeCanvas, new ShapeJXLabelDropTargetListener(this,type)));
	...
	protected String modExpression(String expression) {
		...
		if( mathShape instanceof IMathEllipse )
			lowerCaseExpression = StringUtils.replace(lowerCaseExpression, IMathEllipse.RADIUS, Double.toString(((IMathEllipse)mathShape).getRadius()));
		else if( mathShape instanceof IMathRectangle ) {
			
			lowerCaseExpression = StringUtils.replace(lowerCaseExpression, IMathRectangle.WIDTH, Double.toString(((IMathRectangle)mathShape).getWidth()));
			lowerCaseExpression = StringUtils.replace(lowerCaseExpression, IMathRectangle.HEIGHT, Double.toString(((IMathRectangle)mathShape).getHeight()));
		} else if( mathShape instanceof IMathTriangle ) {
			lowerCaseExpression = StringUtils.replace(lowerCaseExpression, IMathTriangle.WIDTH, Double.toString(((IMathTriangle)mathShape).getWidth()));
			lowerCaseExpression = StringUtils.replace(lowerCaseExpression, IMathTriangle.HEIGHT, Double.toString(((IMathTriangle)mathShape).getHeight()));
		} else if( mathShape instanceof IMathRectangle3D ) {
			lowerCaseExpression = StringUtils.replace(lowerCaseExpression, IMathRectangle3D.WIDTH, ((IMathRectangle3D)mathShape).getWidthValue());
			lowerCaseExpression = StringUtils.replace(lowerCaseExpression, IMathRectangle3D.HEIGHT, ((IMathRectangle3D)mathShape).getHeightValue());
		} else if( mathShape instanceof IMathCylinder3D ) {
			lowerCaseExpression =StringUtils.replace(lowerCaseExpression, IMathCylinder3D.RADIUS, ((IMathCylinder3D)mathShape).getRadiusValue());
			lowerCaseExpression = StringUtils.replace(lowerCaseExpression, IMathCylinder3D.HEIGHT, ((IMathCylinder3D)mathShape).getHeightValue());
		}
	...
	protected void selectInTable(IMathShape mathShape) {
		...
		table.getSelectionModel().setSelectionInterval(Integer.parseInt(mathShape.getId()), Integer.parseInt(mathShape.getId()));
	}
	...
	public void addShape(JLabel label, Point dropPoint, String type) {
		...
		if( label.getName().equals(UIUtils.CIRCLE)) {
			...
			MathRectangle t = new MathRectangle(dropPoint.x, dropPoint.y, 100, 100);
			sc.addShape(t);
			this.setSelectedMathShape(t);
	...
	Action add3dAction = new AbstractAction() {
		...
		public void actionPerformed(ActionEvent e) {
				...
				Math3DShape mathShape = (Math3DShape) button.getClientProperty(UIUtils.SHAPE);
	...
	Action addResultAction = new AbstractAction("Add") {
		...
		public void actionPerformed(ActionEvent ae) {
			...
			DefaultTableModel model = (DefaultTableModel) getComputationTables().get(t).getModel();
			
			model.addRow(new Object[]{new Integer(model.getRowCount() + 1),getMathSelectedShape().getType() ,new Float(text),new Float(text)});
			getMathSelectedShape().setId(Integer.toString((model.getRowCount()-1)));
...
//			float parseFloat = Float.parseFloat(resultLabel.getText());
	...
	ActionListener equalsAction = new ActionListener() {
		@Override
		public void actionPerformed(ActionEvent e) {
			JXButton equalsButton = (JXButton) e.getSource();
			String calcType = (String) equalsButton.getClientProperty(UIUtils.TYPE);
