...
/**
 * This class provides some tests to check if the {@link AgentManager} is working correctly. It uses the conventions of the JUnit Framework.
 * 
...
public class AgentManagerTest {
    ...
    /**
     * This method is called after the tests are finished. It stops the TupleSpace-{@link Server} if running in standalone-mode.
     */
    @AfterClass
     ...
     *             Is thrown if something went wrong during {@code Thread.sleep()}.
     * @throws AgentLifecycleException
     *             Is thrown if something went wrong during {@code AgentManager.startAgent()} or {@code AgentManager.stopAgent()}.
     */
     ...
     *             Is thrown if something went wrong inside the {@link TupleSpace}.
     */
    @SuppressWarnings("deprecation")
    @Test
    ...
    /**
     * This test tries to test the {@code ALIVE} functionality of an agent using the {@link AgentManager} to start the agent. <br />
     * After started the agent this test awaits an {@link Tuple} with an {@code ALIVE} response of the agent.
     * 
     * @throws AgentLifecycleException
     *             Is thrown if something went wrong during {@code AgentManager.startAgent()} or {@code AgentManager.stopAgent()}.
     * @throws InterruptedException
     ...
     * After started and killed the agent this test awaits an {@link Tuple} with an {@code ALIVE} response of the agent and assert that this tuple does not exist.
     * 
     * @throws AgentLifecycleException
     *             Is thrown if something went wrong during {@code AgentManager.startAgent()} or {@code AgentManager.killAgent()}.
     * @throws InterruptedException
     *             Is thrown if something went wrong during {@code Thread.sleep()}.
     * @throws TupleSpaceException
     *             Is thrown if something went wrong inside the {@link TupleSpace}.
    ...
    public void testAgentMaps() throws AgentLifecycleException, InterruptedException {
        ...
        Thread.sleep(1000);
        // After the agents are stopped there should be 0 agents inside the getAgentsIdMap-Map and 7 agents inside getOldAgentsMap-Map.
        assertEquals(0, agentManager.getAgentsIdMap().size());
