...
public class HypothesisEvaluationAgent extends AbstractELOSavedAgent implements IRepositoryAgent {
  ...
  public void processELOSavedAction(String actionId, String user, long timeInMillis,
    ...
    try {
      ...
      IMetadataKey metadataKeywordsKey = metadataTypeManager.getMetadataKey(keywordsKey);
      ...
      List<KeyValuePair> keywordPairs = (List<KeyValuePair>) metadataValueContainer.getValueList();
  ...
  private void addSentenceHistogramToMetadata(IELO elo, HashMap<Integer, Integer> histogram) {
    ...
    // repository.updateELO(elo); //XXX this is important because an update would change the version
  ...
  private IMetadata addKeywordsToMetadata(IELO elo, List<String> keywords) {
    ...
    // return updateELO;
    // XXX this is important because an update would change the version number of the elo, which
