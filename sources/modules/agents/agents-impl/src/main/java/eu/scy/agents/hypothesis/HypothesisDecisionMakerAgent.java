 ...
 * related tool: COPEX
 * <p/>
 * Hypothesis evaluation rules: the histogram that is evaluated contains information about the number of topics per
 * sentence: a histogram like "{0=16, 1=14, 2=7, 3=5, 4=1, 5=1}" indicates that the hypothesis text contains 16
 * sentences without keyword, 14 sentences containing one keyword, ..., one sentence containing 5 keywords. To
 * evaluate,
 * we sum up the numbers of sentences, called N. Furthermore we score sentences that contain keywords: Sentences
 * without
 * keyword score 0, sentences with one, two, ... keywords score 1, 2, ... So we get a second number M by summing up the
 * scores per keyword frequency. Then we compute the fraction M/N and set a threshold to evaluate it.
 * <p/>
 * A second criterion is the co-ocurrence of keywords in one sentence, which also is checked and should be above a
 * minimum of cases.
 *
    ...
    public void call(Command command, int seq, Tuple hypothesisHistogramTuple,
        ...
        if (hypothesisHistogramTuple != null) {
            try {
                ...
                String language = getSession().getLanguage(contextInformation.user);
                ...
                if (score < 0.6) {
                    ...
                    sendNotification(contextInformation, eloUri,
                            messages.getString("HYPO_TOO_FEW_KEYWORDS"));
                } else if (maxKey < 2) {
                    // send notification
                    // " try to express more inter-relation between keywords"
                    sendNotification(contextInformation, eloUri,
                            messages.getString("HYPO_RELATION"));
                } else {
