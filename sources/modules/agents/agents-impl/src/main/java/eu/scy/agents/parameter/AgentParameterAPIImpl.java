...
public class AgentParameterAPIImpl implements AgentParameterAPI {
    ...
    public AgentParameterAPIImpl() throws TupleSpaceException {
        this("localhost", Configuration.getConfiguration().getNonSSLPort());
    }

    public AgentParameterAPIImpl(String tsHost, int tsPort) throws TupleSpaceException {
        this(new TupleSpace(new User(TS_USER), tsHost, tsPort, true, false, AgentProtocol.COMMAND_SPACE_NAME));
    }
    ...
    private <T> Tuple createSetParameterTupleTemplate(String agentName, String mission, String user, String parameterName, T value) {
        Tuple setParameterTuple = AgentProtocol.getParameterSetTupleTemplate();
    ...
    private <T> Tuple createGetParameterTupleTemplate(String agentName, String mission, String user, String parameterName) {
        Tuple getParameterTuple = AgentProtocol.getParameterGetQueryTupleTemplate();
    ...
    private <T> T getParameterViaTuple(String agentName, String mission, String user, String parameterName) {
        Tuple getParameterTuple = createGetParameterTupleTemplate(agentName, mission, user, parameterName);
        try {
            tupleSpace.write(getParameterTuple);
            Tuple getParameterResponse = tupleSpace.waitToRead(AgentProtocol.getParameterGetResponseTupleTemplate(agentName), AgentProtocol.MINUTE);
    ...
    public <T> T getParameter(String agentName, AgentParameter parameter) {
        return (T) getParameterViaTuple(agentName, parameter.getMission(), parameter.getUser(), parameter.getParameterName());
    }

    @Override
    public <T> void setParameter(String agentName, AgentParameter parameter) {
        try {
            Tuple setParameterTuple = createSetParameterTupleTemplate(agentName, parameter.getMission(), parameter.getUser(), parameter.getParameterName(), parameter.getParameterValue());
            tupleSpace.write(setParameterTuple);
        } catch (TupleSpaceException e) {
            e.printStackTrace();
            throw new RuntimeException("Couldn't set parameter" + parameter.getParameterName() + " for " + agentName, e);
        }
    }
    ...
    public List<String> listAgentParameter(String agentName) {
        ...
        Tuple listParameterTuple = AgentProtocol.getListParametersTupleQuery(agentName, queryId.toString());
        try {
            ...
            Tuple responseTuple = tupleSpace.waitToRead(responseTupleFormat, 2 * AgentProtocol.MINUTE);
            List<String> parameterNames = new ArrayList<String>();
            for (int i = 4; i < responseTuple.getNumberOfFields(); i++) {
                parameterNames.add((String) responseTuple.getField(i).getValue());
            }
