...
public class ProcessGuidanceAgent extends AbstractThreadedAgent {
	...
	MissionModel aModel = new MissionModel(getCommandSpace(), guidanceSpace, "roolo://scy.collide.info/scy-collide-server/289.289#0", "pizzaMission");
	...
	MissionModel[] newMissionModels = (MissionModel[]) Arrays.copyOf(missionModels, missionModels.length + 1);
	MissionModel aModel = new MissionModel(getCommandSpace(), guidanceSpace, "roolo://scy.collide.info/scy-collide-server/xxx.xxx#0", "ecoMission");
	...
	MissionModel[] newMissionModels = (MissionModel[]) Arrays.copyOf(missionModels, missionModels.length + 1);
	MissionModel aModel = new MissionModel(getCommandSpace(), guidanceSpace, "roolo://scy.collide.info/scy-collide-server/xxx.xxx#0", "co2MissionRedesign");
    ...
    private void handleDataEdited(Action action) {
	RunUser aRunUser = users.get(action.getUser());
	String elo_uri = action.getContext(ContextConstants.eloURI);
	ELORun aELORun = aRunUser.getMissionRun().findELORunByURI(elo_uri);
	if (aELORun != null) {
	    aELORun.handleDataEdited(aRunUser, action.getTimeInMillis());
	    aRunUser.getActionHistory().addAction(new UserAction(UserAction.ACTION_DATA_EDITED, aELORun, action.getTimeInMillis()));
	}
    }

    private void handleValueChanged(Action action) {
	RunUser aRunUser = users.get(action.getUser());
	String elo_uri = action.getContext(ContextConstants.eloURI);
	ELORun aELORun = aRunUser.getMissionRun().findELORunByURI(elo_uri);
	if (aELORun != null) {
...
//	ArrayList<ELORun> openedELORun = aRunUser.getOpenedELORun();
//	for (ELORun eloRun : openedELORun) {
//	    System.out.println();
//	}
	...
	if (aELORun == null) {
	    ...
	    // the user starts a closed, used tool in the same session, nothing
	    // happens
	}
    }
	...
	// I don't know why the last logout action will be received when login
	// again
	if (aRunUser == null)
	    return;
	aRunUser.updateUserTuple(); // change the last access time
	aRunUser.getMissionRun().updateMissionRunTuple(); // change the last
	                                                  // access time
	users.remove(userID);
    }
...
*/
	    ...
	    } else if (actionType.equals("data_edited")) {
