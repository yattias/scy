...
public class OntologyKeywordsAgent extends AbstractRequestAgent {
  ...
  public OntologyKeywordsAgent(Map<String, Object> params) {
    ...
    this.activationTuple = new Tuple(EXTRACT_ONTOLOGY_KEYWORDS, AgentProtocol.QUERY, String.class,
                                     String.class);
    try {
      this.listenerId = this.getCommandSpace().eventRegister(Command.WRITE, this.activationTuple,
                                                             this, true);
  ...
  private Tuple createResponseTuple(Set<String> keywords, String queryId) {
    ...
    return new Tuple(EXTRACT_ONTOLOGY_KEYWORDS, AgentProtocol.RESPONSE, queryId,
  ...
  private Set<String> getOntologyKeywords() {
    ...
    try {
      ...
      this.getCommandSpace().write(new Tuple(queryId.toString(), "onto", "entities",
                                             "http://www.scy.eu/co2house#"));

      Tuple response = this.getCommandSpace().waitToTake(new Tuple(queryId.toString(),
