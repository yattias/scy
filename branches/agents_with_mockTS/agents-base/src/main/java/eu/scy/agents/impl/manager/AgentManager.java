...
public class AgentManager implements Callback {
	...
	public AgentManager(String host, int port) {
		this(new SQLSpacesFactory(), host, port);
	}

	public AgentManager(TupleSpaceFactory tsFactory, String host, int port) {
		initLogger();
		...
		tupleSpaceFactory = tsFactory;

		try {
			// TODO COmmand.all
			tupleSpace = tupleSpaceFactory.createTupleSpace("AgentManager",
					host, port, AgentProtocol.COMMAND_SPACE_NAME);
	 ...
	 */
	Map<String, IThreadedAgent> getOldAgentsMap() {
		return oldAgents;
	}
	 ...
	 */
	Map<String, IThreadedAgent> getAgentsIdMap() {
		return agentIdMap;
	}
	...
	public void setTupleSpaceFactory(TupleSpaceFactory tsFactory) {
		tupleSpaceFactory = tsFactory;
	}
}
