...
public abstract class AbstractAgent implements IAgent {
	...
	public TupleSpace getTupleSpace() {
		if (tupleSpace == null) {
			//			try {
				// tupleSpace = new SQLTupleSpace(new User(getName()), host,
				// port,
				// runAutonomous, false, AgentProtocol.COMMAND_SPACE_NAME);

				if (runAutonomous) {
					tupleSpace = tupleSpaceFactory.createAutonomousTupleSpace(
							getName(), host, port,
							AgentProtocol.COMMAND_SPACE_NAME);
				} else {
					tupleSpace = tupleSpaceFactory.createTupleSpace(getName(),
